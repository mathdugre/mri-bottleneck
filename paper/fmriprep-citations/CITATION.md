...
with `N4BiasFieldCorrection` [@n4], distributed with ANTs 2.4.2 [@ants, RRID:SCR_004757], and used as T1w-reference throughout the workflow.
The T1w-reference was then skull-stripped with a *Nipype* implementation of
the `antsBrainExtraction.sh` workflow (from ANTs), using OASIS30ANTs
as target template.
Brain tissue segmentation of cerebrospinal fluid (CSF),
white-matter (WM) and gray-matter (GM) was performed on
the brain-extracted T1w using `fast` [FSL 6.0.6.4, RRID:SCR_002823,
@fsl_fast].
Brain surfaces were reconstructed using `recon-all` [FreeSurfer 7.3.3-20231213-debug,
...
nonlinear registration with `antsRegistration` (ANTs 2.4.2),
...
`mcflirt` [FSL 6.0.6.4, @mcflirt].
BOLD runs were slice-time corrected to 0.961s (0.5 of slice acquisition range
0s-1.92s) using `3dTshift` from AFNI 20230309 [@afni, RRID:SCR_005927].
