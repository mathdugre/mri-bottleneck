...
#SBATCH -J fmriprep-anat-anat-nofs
#SBATCH --array=1
...
source ./sbatch/pre_run.sh fmriprep anat-nofs -j ${SLURM_CPUS_PER_TASK} $@
...
# Extra cleanup
mkdir -p ${SLURM_TMPDIR}/derivatives/fmriprep/anat-nofs/sub-${SUBJECT_ID}
rsync -aLq --info=progress2 \
    ${SUBJECT_OUTPUT}/ \
    ${SLURM_TMPDIR}/derivatives/fmriprep/anat-nofs/sub-${SUBJECT_ID}/
