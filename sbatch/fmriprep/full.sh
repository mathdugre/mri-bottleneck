...
source ./sbatch/pre_run.sh fmriprep full -j ${SLURM_CPUS_PER_TASK} $@
...
# Extra cleanup
mkdir -p ${SLURM_TMPDIR}/derivatives/fmriprep/full/sub-${SUBJECT_ID}
rsync -aLq --info=progress2 \
    ${SUBJECT_OUTPUT}/ \
    ${SLURM_TMPDIR}/derivatives/fmriprep/full/sub-${SUBJECT_ID}/
